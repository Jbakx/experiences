...
folder <- paste(main_folder, "/resultaten/final/timeline", sep = "")     
load(paste(folder, "/oddsratios GC and EC events in EC and GC patients.Rdata", sep = ""))

outcomes <- outcome_model

outcomes$timeline    <- stringr::str_extract(outcomes$event, "(\\d)+(?=days_high)")
outcomes$timeline    <- paste(outcomes$timeline, "days", sep = "")
outcomes$events      <- sapply(stringr::str_split(outcomes$event, pattern = outcomes$timeline), `[`, 1)
outcomes$events      <- sub("ratio_", "", outcomes$events)
outcomes$p_signif    <- ifelse(outcomes$p_value < 0.001, '**', ifelse(outcomes$p_value < 0.01, '*', ''))
outcomes$timeline    <- sub("([a-z])", " \\1", outcomes$timeline)
outcomes$timeline    <- factor(outcomes$timeline,  levels = c("90 days", "60 days", "30 days", "10 days", "5 days") )


#order dataframe to match the labels for the plot
outcomes <- outcomes[with(outcomes, order(event, timeline)),] 
...
subtitle_name <- "based on the number of events in previous timeline with EC and GC"


plot_EC_and_GC <- ggplot(outcomes, aes(x=OR, y=boxLabels, xmin=lower_CI, xmax=upper_CI, col=timeline, fill=timeline)) +
  ...
  geom_text(aes(x = upper_CI, label = p_signif, group = timeline), position=position_dodge(width = 0.6), hjust = -1, color = "black") +
...
plot_EC <- ggplot(outcomes, aes(x=OR, y=boxLabels, xmin=lower_CI, xmax=upper_CI, col=timeline, fill=timeline)) +
  geom_vline(aes(xintercept = 1), size = .25, linetype = 'dashed', color = "darkgrey") +
  geom_errorbarh(aes(x = upper_CI, xmin = lower_CI), size = .5, height =
                   .2, color = 'gray50', position=position_dodge(width = 0.6)) +
  geom_text(aes(x = upper_CI, label = p_signif, group = timeline), position=position_dodge(width = 0.6), hjust = -1, color = "black") +
...
plot_GC <- ggplot(outcomes, aes(x=OR, y=boxLabels, xmin=lower_CI, xmax=upper_CI, col=timeline, fill=timeline)) +
  geom_vline(aes(xintercept = 1), size = .25, linetype = 'dashed', color = "darkgrey") +
  geom_errorbarh(aes(x = upper_CI, xmin = lower_CI), size = .5, height =
                   .2, color = 'gray50', position=position_dodge(width = 0.6)) +
  geom_text(aes(x = upper_CI, label = p_signif, group = timeline), position=position_dodge(width = 0.6), hjust = -1, color = "black") +
...
folder <- (paste(main_folder, "resultaten/final/timeline/plots", sep = "/"))     
...
# SUPPLEMENTARY PLOTS_______________________________________________________________________________________________________
...
subtitle_name <- "based on the number of events in patients with EC in the past days"

plot_EC_sup <- ggplot(outcomes, aes(x=OR, y=boxLabels, xmin=lower_CI, xmax=upper_CI, col=timeline, fill=timeline)) +
  geom_vline(aes(xintercept = 1), size = .25, linetype = 'dashed', color = "darkgrey") +
  geom_errorbarh(aes(x = upper_CI, xmin = lower_CI), size = .5, height =
                   .2, color = 'gray50', position=position_dodge(width = 0.6)) +
  geom_text(aes(x = upper_CI, label = p_signif, group = timeline), position=position_dodge(width = 0.6), hjust = -1, color = "black") +
...
subtitle_name <- "based on the number of events in patients with GC in the past days"

plot_GC_sup <- ggplot(outcomes, aes(x=OR, y=boxLabels, xmin=lower_CI, xmax=upper_CI, col=timeline, fill=timeline)) +
  geom_vline(aes(xintercept = 1), size = .25, linetype = 'dashed', color = "darkgrey") +
  geom_errorbarh(aes(x = upper_CI, xmin = lower_CI), size = .5, height =
                   .2, color = 'gray50', position=position_dodge(width = 0.6)) +
  geom_text(aes(x = upper_CI, label = p_signif, group = timeline), position=position_dodge(width = 0.6), hjust = -1, color = "black") +
  ...
  theme_bw() +
  theme(panel.grid.minor = element_blank()) +
  coord_cartesian(xlim = c(0.2, 4.0)) +
  scale_x_log10(breaks = c(0.2, 0.4, 0.6, 0.8, 1, 1.5, 2, 3, 4), labels = c("0.3", "0.4", "0.6", "0.8", "1", "1.5", "2", "3", "4")) +
  ylab('') +
  xlab('Odds ratio (log scale)') +
  ggtitle(label = title_name, subtitle = subtitle_name) +
  guides(fill = guide_legend(reverse = TRUE))  +
